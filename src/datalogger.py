...
class DataLogger:
    ...
    def setup_adc(self, capture_limit: int) -> None:
        self.send_command("l", capture_limit)
    ...
    def process_user_input(self, cmd_input, capture_limit):
        if cmd_input == "t":
            ...
            self.flush_buffer()
            self.setup_adc(capture_limit=capture_limit)
            data = self.trigger_adc(timeout=timeout)
